        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="card-modern overflow-hidden animate-pulse">
              ...
              <div className="p-5 space-y-2"><div className="h-4 skeleton rounded w-2/3" /><div className="h-3 skeleton rounded w-full" /><div className="h-3 skeleton rounded w-1/2" /></div>
            </div>
          ))}
        </div>
      ) : blogs.length === 0 ? (
        <div className="text-center py-16"><h3 className="text-lg font-medium">No blog posts found</h3><p className="text-muted mt-1">{search ? 'Try a different search term' : 'Check back soon for updates'}</p></div>
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {blogs.map((blog) => (
