...
import { Ruler, RotateCcw, Plus, X } from 'lucide-react';
      ...
      <div className="max-w-2xl mx-auto">
            ...
            <div key={i} className="flex gap-3 items-end">
              <div className="flex-1">
                ...
                <input
                  type="number"
                  value={wall.height || ''}
                  onChange={(e) => updateWall(i, 'height', parseFloat(e.target.value) || 0)}
              ...
              <div className="flex-1">
                ...
                <input
                  type="number"
                  value={wall.width || ''}
                  onChange={(e) => updateWall(i, 'width', parseFloat(e.target.value) || 0)}
                  className="input-modern text-sm"
                  placeholder="0"
                />
              </div>
              {walls.length > 1 && (
                <button onClick={() => removeWall(i)} className="btn-ghost p-2 text-red-500">
                  <X size={16} />
                </button>
              )}
            </div>
          ))}
          <button onClick={addWall} className="btn-ghost text-sm">
            <Plus size={16} /> Add another wall
          </button>
        ...
        {result && (
          <div className="card-modern p-6 mt-6">
            ...
            <p className="text-3xl font-bold text-gradient">{result.totalArea.toFixed(2)} sq ft</p>
