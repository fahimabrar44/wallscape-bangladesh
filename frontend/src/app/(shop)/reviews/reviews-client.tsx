...
function StarRating({ rating, size = 16 }: { rating: number; size?: number }) {
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((s) => (
        <svg key={s} width={size} height={size} viewBox="0 0 24 24" className={s <= rating ? 'text-primary-light fill-primary-light' : 'text-gray-200 fill-gray-200'}>
...
export default function ReviewsClient() {
  ...
  const submitMutation = useMutation({
    ...
    onSuccess: () => { toast.success('Thank you! Your review has been submitted for approval.'); setShowForm(false); setForm({ customerName: '', rating: 5, review: '' }); },
    onError: (err: any) => toast.error(err?.message || 'Failed to submit review'),
  });

  const apiReviews = (apiData?.reviews || []).map((r: any) => ({
    name: r.customerName, location: '', rating: r.rating, text: r.review,
    initials: r.customerName.split(' ').map((n: string) => n[0]).join('').substring(0, 2).toUpperCase(),
    date: new Date(r.createdAt).toLocaleDateString('en-US', { year: 'numeric', month: 'long' }), tags: [],
  }));
    ...
    <div className="min-h-screen">
        ...
        <div className="container-custom relative z-10 py-16 sm:py-20 lg:py-28">
          <div className="text-center max-w-2xl mx-auto">
            <div className="inline-flex items-center gap-2 bg-white/10 px-4 py-1.5 rounded-full text-sm mb-5">
              <MessageCircle size={14} /><span>{totalReviews} Verified Reviews</span>
            </div>
            <h1 className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-3">What Our Customers Say</h1>
            <p className="text-gray-300 text-sm sm:text-base max-w-lg mx-auto">Real stories from real people who transformed their spaces with WALLSCAPE</p>
          </div>
        </div>
      </section>
        ...
        <div className="container-custom py-6 sm:py-8">
          <div className="flex flex-col sm:flex-row items-center gap-6 sm:gap-10">
            <div className="flex items-center gap-4">
              ...
              <div><p className="text-2xl font-bold">{avgRating}</p><p className="text-xs text-muted">Average Rating</p></div>
            </div>
              ...
              {[5, 4, 3].map((star) => (
                <div key={star} className="flex items-center gap-2 text-sm">
                  ...
                  <div className="w-20 sm:w-24 h-2 bg-gray-100 rounded-full overflow-hidden">
                    <div className="h-full bg-gradient-to-r from-primary-light to-primary-light rounded-full"
      ...
      <section className="bg-white border-b border-border">
        <div className="container-custom py-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            <div className="flex items-center gap-2 text-sm text-muted"><Filter size={14} /><span>Filter:</span></div>
            <div className="flex flex-wrap gap-1.5">
              <button onClick={() => setFilterTag('')} className={`px-3 py-1.5 rounded-full text-xs font-medium transition ${!filterTag ? 'bg-primary text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>All</button>
              {allTags.map((tag) => (
                <button key={tag} onClick={() => setFilterTag(tag)} className={`px-3 py-1.5 rounded-full text-xs font-medium transition ${filterTag === tag ? 'bg-primary text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>{tag}</button>
              ))}
            </div>
            <div className="sm:ml-auto">
              <select value={sortBy} onChange={(e) => setSortBy(e.target.value)} className="text-sm border border-border rounded-lg px-3 py-1.5 bg-white focus:outline-none focus:ring-2 focus:ring-primary/20">
      ...
      <section className="py-10 sm:py-16">
        <div className="container-custom">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filtered.map((r, idx) => (
              <div key={idx} className="card-modern p-6 lg:p-7">
                <div className="flex items-center justify-between mb-3"><StarRating rating={r.rating} /><span className="text-xs text-muted">{r.date}</span></div>
                ...
                <div className="flex items-center gap-3 pt-3 border-t border-border/50">
                  <div className="w-10 h-10 bg-gradient-to-br from-primary to-primary-light rounded-full flex items-center justify-center text-white text-sm font-bold shrink-0">{r.initials}</div>
                  <div className="min-w-0">
                    <p className="font-semibold text-sm">{r.name}</p>
                    {r.location && <p className="text-xs text-muted flex items-center gap-1 truncate"><MapPin size={10} className="shrink-0" />{r.location}</p>}
                  </div>
                </div>
                {r.tags.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-3 pt-3 border-t border-border/50">
                    {r.tags.map((tag: string) => (
                      <button key={tag} onClick={() => setFilterTag(tag)} className="text-[10px] px-2 py-0.5 bg-primary/5 text-primary rounded-full font-medium hover:bg-primary/10 transition">{tag}</button>
                    ))}
                  </div>
                )}
              </div>
          ...
          {filtered.length === 0 && (
            <div className="text-center py-16"><MessageCircle size={40} className="mx-auto text-gray-300 mb-3" /><p className="text-muted">No reviews match this filter.</p></div>
          )}

          <div className="text-center mt-12">
            <button onClick={() => setShowForm(true)} className="inline-flex items-center gap-2 bg-primary text-white px-6 py-3 rounded-lg font-medium hover:bg-primary-dark transition shadow-md hover:shadow-lg">
      ...
      <section className="border-t border-border">
        <div className="container-custom py-4">
          <nav className="flex items-center gap-2 text-sm text-muted">
            ...
            <ChevronRight size={12} /><span className="text-dark font-medium">Customer Reviews</span>
          </nav>
        </div>
      </section>
    </div>
