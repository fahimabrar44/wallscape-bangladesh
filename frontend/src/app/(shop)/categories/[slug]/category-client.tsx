...
import { ArrowUpDown, SearchX, ShoppingCart } from 'lucide-react';
...
export default function CategoryClient({ category, slug }: { category: Category | null; slug: string }) {
  ...
  return (
    <>
      ...
      <div className="flex items-center justify-between mb-5">
        <p className="text-sm text-muted">{pagination?.total || 0} product{pagination?.total !== 1 ? 's' : ''} found</p>
      ...
      {prodLoading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-6">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="card-modern animate-pulse overflow-hidden">
              <div className="aspect-[3/4] skeleton" /><div className="p-4 space-y-2"><div className="h-3 skeleton rounded w-1/3" /><div className="h-4 skeleton rounded w-2/3" /><div className="h-4 skeleton rounded w-1/2" /></div>
            </div>
          ))}
        </div>
      ) : products.length === 0 ? (
        <div className="text-center py-20">
          ...
          <Link href="/products" className="inline-flex items-center gap-1 text-primary font-medium hover:underline text-sm">Browse all products &rarr;</Link>
        </div>
      ) : (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-6">
          ...
          {pagination && pagination.pages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-10">
              {page > 1 && <button onClick={() => setPage(page - 1)} className="px-4 h-10 rounded-lg border border-border text-sm hover:border-primary transition">&larr; Prev</button>}
              {Array.from({ length: Math.min(pagination.pages, 8) }).map((_, i) => (
                <button key={i} onClick={() => setPage(i + 1)} className={`w-10 h-10 rounded-lg text-sm font-medium transition ${page === i + 1 ? 'bg-primary text-white shadow-md' : 'bg-white border border-border hover:border-primary'}`}>{i + 1}</button>
              ))}
              {page < pagination.pages && <button onClick={() => setPage(page + 1)} className="px-4 h-10 rounded-lg border border-border text-sm hover:border-primary transition">Next &rarr;</button>}
            </div>
          )}
        </>
      )}
...
function ProductCard({ product }: { product: Product }) {
  ...
  function handleAddToCart(e: React.MouseEvent) { e.preventDefault(); e.stopPropagation(); if (product.stock <= 0) return; addItem(product); toast.success(`"${product.name}" added to cart`); }
    ...
    <div className="group relative">
      <Link href={`/products/${product.slug}`} className="block bg-white rounded-2xl border border-border overflow-hidden hover:border-primary/20 hover:shadow-xl transition-all duration-500">
        <div className="relative aspect-[3/4] bg-gray-100 overflow-hidden">
          ...
          {product.discountPrice && <span className="badge-accent absolute top-2 left-2 text-[10px]">-{Math.round(((product.price - product.discountPrice) / product.price) * 100)}%</span>}
          {product.stock <= 0 && (
            <div className="absolute inset-0 bg-black/50 backdrop-blur-[2px] flex items-center justify-center">
              <span className="bg-white text-dark font-semibold px-4 py-1.5 rounded-lg text-xs shadow-lg">Out of Stock</span>
            </div>
          )}
          <div className="absolute inset-x-3 bottom-3 opacity-0 group-hover:opacity-100 translate-y-2 group-hover:translate-y-0 transition-all duration-300">
            <span onClick={handleAddToCart} className="block w-full text-center text-xs font-medium bg-white/90 backdrop-blur-sm text-dark py-2 rounded-lg shadow-lg cursor-pointer hover:bg-white transition">
        ...
        <div className="p-3 lg:p-4">
          ...
          <div className="flex items-center gap-2 mt-2">
            <span className="price-current text-sm">{formatCurrency(product.discountPrice || product.price)}</span>
            {product.discountPrice && <span className="price-original text-[11px]">{formatCurrency(product.price)}</span>}
          </div>
        </div>
      </Link>
    </div>
