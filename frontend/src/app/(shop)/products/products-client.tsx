...
import { SlidersHorizontal, Search, Star, X, Heart, ShoppingCart } from 'lucide-react';
...
export default function ProductsContent() {
  ...
  function clearFilter(key: string) { setFilters((prev) => ({ ...prev, [key]: '' })); setPage(1); }
  function clearAll() { setFilters({ category: '', sort: 'newest', search: '', minPrice: '', maxPrice: '', material: '', color: '', pattern: '' }); setPage(1); }
          ...
          <div className={`${showFilters ? 'bg-white p-6 w-full max-w-sm mx-auto mt-16 rounded-t-2xl shadow-2xl overflow-y-auto max-h-[calc(100vh-4rem)]' : ''} lg:block`}>
            ...
            <div className="lg:card-modern lg:p-5 lg:sticky lg:top-24 space-y-6">
              ...
              <div>
                ...
                <div className="space-y-1">{materialOptions.map((m) => (<button key={m} onClick={() => { setFilters({ ...filters, material: filters.material === m ? '' : m }); setPage(1); }} className={`block text-sm w-full text-left px-3 py-2 rounded-lg transition-all ${filters.material === m ? 'bg-primary/10 text-primary font-medium' : 'hover:bg-gray-50 text-gray-700'}`}>{m}</button>))}</div>
              </div>
              <hr className="border-border/50" />
              <div>
                <h3 className="section-label mb-3">Color</h3>
                <div className="flex flex-wrap gap-2">{colorOptions.map((c) => (<button key={c} onClick={() => { setFilters({ ...filters, color: filters.color === c ? '' : c }); setPage(1); }} className={`text-xs px-3 py-1.5 rounded-full border transition-all ${filters.color === c ? 'border-primary bg-primary text-white' : 'border-border hover:border-primary/50 text-gray-700'}`}>{c}</button>))}</div>
              </div>
              <hr className="border-border/50" />
              <div>
                <h3 className="section-label mb-3">Pattern</h3>
                <div className="flex flex-wrap gap-2">{patternOptions.map((s) => (<button key={s} onClick={() => { setFilters({ ...filters, pattern: filters.pattern === s ? '' : s }); setPage(1); }} className={`text-xs px-3 py-1.5 rounded-full border transition-all ${filters.pattern === s ? 'border-primary bg-primary text-white' : 'border-border hover:border-primary/50 text-gray-700'}`}>{s}</button>))}</div>
              </div>
              <hr className="border-border/50" />
              <div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 lg:gap-5">
              {Array.from({ length: 12 }).map((_, i) => (
                <div key={i} className="card-modern overflow-hidden animate-pulse">
                  ...
                  <div className="p-4 space-y-3"><div className="h-3 skeleton rounded w-1/3" /><div className="h-4 skeleton rounded w-2/3" /><div className="h-4 skeleton rounded w-1/2" /></div>
                </div>
              ))}
            </div>
          ) : products.length === 0 ? (
            ...
            <>
              ...
              {pagination && pagination.pages > 1 && (
                <div className="flex items-center justify-center gap-2 mt-12">
                  {page > 1 && <button onClick={() => setPage(page - 1)} className="px-4 h-10 rounded-xl border border-border text-sm hover:border-primary transition-all">&larr; Prev</button>}
                  {Array.from({ length: Math.min(pagination.pages, 8) }).map((_, i) => (<button key={i} onClick={() => setPage(i + 1)} className={`w-10 h-10 rounded-xl text-sm font-medium transition-all ${page === i + 1 ? 'bg-primary text-white shadow-md' : 'border border-border hover:border-primary text-gray-700'}`}>{i + 1}</button>))}
                  {page < pagination.pages && <button onClick={() => setPage(page + 1)} className="px-4 h-10 rounded-xl border border-border text-sm hover:border-primary transition-all">Next &rarr;</button>}
                </div>
              )}
            </>
          )}
