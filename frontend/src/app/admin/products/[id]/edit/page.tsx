...
export default function EditProductPage() {
  ...
  const { data: productData, isLoading: productLoading } = useQuery({
    queryKey: ['product', id],
    queryFn: () => api.get<{ product: Product }>(`/api/products/${id}/by-id`),
    enabled: !!id,
  });
  ...
  useEffect(() => {
    if (productData?.product) {
      ...
      setForm({
        ...
        isInstallationAvailable: p.isInstallationAvailable,
  ...
  const updateMutation = useMutation({
    mutationFn: (data: any) => api.put(`/api/products/${id}`, data),
    onSuccess: () => {
  ...
  if (productLoading) return <div className="text-center py-12 text-muted">Loading product...</div>;
      ...
      <form onSubmit={handleSubmit} className="space-y-5 max-w-3xl">
        ...
        <Section title="Basic Information" icon={Package}>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={form.productCode} onChange={(e) => updateField('productCode', e.target.value)} className={inputClass} placeholder="Auto-generated if empty" />
            </div>
            <div>
        ...
        <Section title="Pricing & Stock" icon={DollarSign}>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="number" min="0" step="0.01" value={form.discountPrice} onChange={(e) => updateField('discountPrice', e.target.value)} className={inputClass} />
            </div>
            <div>
        ...
        <Section title="Dimensions & Coverage" icon={Ruler}>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={form.rollSize} onChange={(e) => updateField('rollSize', e.target.value)} className={inputClass} />
            </div>
            <div>
              <label className="block text-sm font-medium mb-1">Coverage Area</label>
              <input type="text" value={form.coverageArea} onChange={(e) => updateField('coverageArea', e.target.value)} className={inputClass} />
            </div>
          </div>
        </Section>
        ...
        <Section title="Description & Specs" icon={Layers}>
          <div>
            ...
            <textarea value={form.description} onChange={(e) => updateField('description', e.target.value)} rows={4} className={`${inputClass} resize-y`} />
          </div>
          <div>
            <label className="block text-sm font-medium mb-1">Specifications (HTML)</label>
            <textarea value={form.specifications} onChange={(e) => updateField('specifications', e.target.value)} rows={3} className={`${inputClass} resize-y font-mono text-xs`} />
          </div>
        </Section>
